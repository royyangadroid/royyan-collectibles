...
export default function UploadCatalogPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSuccess('Katalog berhasil diunggah dan di-push ke GitHub!');
      form.reset();
      setPreviewData({
        collectionNumber: '',
        title: '',
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-sm font-medium text-zinc-300">Collection Number (e.g. RC-040)</label>
                <input required name="collectionNumber" type="text" onChange={handleInputChange} className="w-full rounded-md border border-zinc-800 bg-zinc-950 px-4 py-2 text-sm text-parchment-100 focus:border-gold focus:outline-none" />
              </div>
